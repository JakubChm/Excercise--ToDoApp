...
const searchTask = ( e ) => {
	e.preventDefault();
	filteredTodoList = todoList.filter( ( todoTask ) => {
		return todoTask.textContent.toLowerCase().includes( searchInput.value.toLowerCase() );
	} );
	filteredDoneList = doneList.filter( ( task ) => {
		return task.textContent.toLowerCase().includes( searchInput.value.toLowerCase() );
	} );
	updateFilteredList();
};
...
const createDoneElement = ( e ) => {
	...
	updateList( doneList, ulDone );
	searchTask( e );
	document.querySelectorAll( '.delete-done-btn' ).forEach( ( btnElement ) => {
		return btnElement.addEventListener( 'click', removeDoneTask );
	} );
	document.querySelectorAll( '.not-done-btn' ).forEach( ( btnElement ) => {
		return btnElement.addEventListener( 'click', moveToTodoList );
	} );
};
const moveToTodoList = ( e ) => {
	...
	updateList( doneList, ulDone );
	searchTask( e );
	document.querySelectorAll( '.delete-btn' ).forEach( ( btnElement ) => {
		return btnElement.addEventListener( 'click', removeTask );
	} );
	document.querySelectorAll( '.done-btn' ).forEach( ( btnElement ) => {
		return btnElement.addEventListener( 'click', createDoneElement );
	} );
};
// Main add function //////////////////////////////
const addTask = ( e ) => {
